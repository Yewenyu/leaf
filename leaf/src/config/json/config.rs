...
pub struct Dns {
    ...
    pub dohkeys:Option<HashMap<String,String>>,
}
...
pub fn to_internal(json: &mut Config) -> Result<internal::Config> {
        ...
        for ext_outbound in ext_outbounds.iter() {
            ...
            match outbound.protocol.as_str() {
                ...
                "failover" => {
                    ...
                    if let Some(v) = ext_settings.health_check_active {
                        settings.health_check_active = v;
                    } else {
                        settings.health_check_active = 15*60;
                    }
    ...
    if let Some(ext_dns) = &json.dns {
        ...
        if let Some(values) = ext_dns.dohkeys.as_ref() {
            let mut newvs = HashMap::new();
            for (k,v) in values.iter() {
                newvs.insert(k.to_string(), v.to_string());
            }
            dns.doh_keys = newvs;
        }
